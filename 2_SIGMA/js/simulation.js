...
window.onRegionChange = function() {
    ...
    // 해당 지역의 교차로들만 사이드바 리스트에 필터링하여 다시 그리기 (구현 필요)
    if (typeof updateJunctionListByRegion === 'function') {
        updateJunctionListByRegion(regionCode);
    }
};
...
async function fetchRealtimeSignals() {
    ...
    for (const j of targets) {
        ...
        const originalUrl = `http://tsihub.utic.go.kr/tsi/api/SigMapCrossRoadInfoService/getSigMapCRInfo?serviceKey=${SERVICE_KEY}&srchCTId=${regionCode}&srchCRNm=${itstNm}&type=json`;
        
        // [IP 권한 문제 해결] 공용 corsproxy.io 대신 로컬 Node.js 프록시 서버(proxy.js) 사용
...
window.updateJunctionListByRegion = async function(regionCode) {
    const listContainer = document.getElementById('j-sidebar-list');
    if (!listContainer) return;
    
    listContainer.innerHTML = '<div class="loading-small">교차로 목록 불러오는 중...</div>';
    
    const SERVICE_KEY = 'kgA1yoaXkd7iVJkKR9Ze5iA7zZ3WWI1UJHn1SQk9QLI';
    const originalUrl = `http://tsihub.utic.go.kr/tsi/api/CrossRoadInfoService/getCRInfo?serviceKey=${SERVICE_KEY}&srchCTId=${regionCode}&type=json&numOfRows=100`;
    const proxyUrl = `http://localhost:3001/proxy?url=${encodeURIComponent(originalUrl)}`;
    
    try {
        const res = await fetch(proxyUrl);
        const data = await res.json();
        const items = Array.isArray(data) ? data.slice(1) : []; // 메타데이터 제외
        
        if (items.length === 0) {
            listContainer.innerHTML = '<div class="text-dim p-10">조회된 교차로가 없습니다.</div>';
            return;
        }
        ...
        items.forEach(item => {
            html += `
                <div class="junction-item-v4" onclick="selectApiJunction('${item.INT_NO}', '${item.INT_NM}', ${item.Y_COORD/10000000}, ${item.X_COORD/10000000})">
                    <span class="j-id">${item.INT_NO}</span>
                    <span class="j-name">${item.INT_NM}</span>
                </div>
            `;
        });
        listContainer.innerHTML = html;
        console.log(`[API] ${regionCode} 지역 교차로 ${items.length}건 로드 완료`);
    } catch (err) {
        console.error("[API] 교차로 목록 로드 실패:", err);
        listContainer.innerHTML = '<div class="text-danger p-10">목록 로드 실패</div>';
    }
};
