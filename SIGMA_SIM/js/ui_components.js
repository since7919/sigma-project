...
function renderAngleFields(jid) {
    const j = jid ? STATE.junctions[jid] : null;
    const grid = document.getElementById('angle-fields-grid');
    if (!grid) return;
    ...
    const directions = [
        { key: 'N', label: '북 (N)', min: -27, max: 27, def: 0 },
        { key: 'NE', label: '북동 (NE)', min: 28, max: 62, def: 45 },
        { key: 'E', label: '동 (E)', min: 63, max: 117, def: 90 },
        { key: 'SE', label: '남동 (SE)', min: 118, max: 152, def: 135 },
        { key: 'S', label: '남 (S)', min: 153, max: 207, def: 180 },
        { key: 'SW', label: '남서 (SW)', min: 208, max: 242, def: 225 },
        { key: 'W', label: '서 (W)', min: 243, max: 297, def: 270 },
        { key: 'NW', label: '북서 (NW)', min: 298, max: 332, def: 315 }
    ];

    let html = '';
    directions.forEach(d => {
        const angle = (j.customAngles && j.customAngles[d.key] !== undefined) ? j.customAngles[d.key] : d.def;
        ...
        html += `
        <div class="info-row-v3" style="display: flex; flex-direction: column; gap: 4px; padding: 6px; border: 1px solid rgba(255,255,255,0.05); background: rgba(255,255,255,0.01);">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="font-size: 11px; font-weight: bold; color: #cbd5e1;">${d.label}</span>
                <span id="txt-angle-${d.key}" style="font-size: 11.5px; font-weight: bold; color: #38bdf8;">${angle}°</span>
            </div>
            <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
                <input type="range" id="slider-angle-${d.key}" min="${d.min}" max="${d.max}" value="${val}" 
                       style="flex: 1; accent-color: #38bdf8; cursor: pointer; height: 4px;"
                       oninput="updateAngleFromSlider('${d.key}', this.value)">
                <input type="number" id="input-angle-${d.key}" min="${d.key === 'N' ? -27 : d.min}" max="${d.key === 'N' ? 333 : d.max}" value="${angle}"
                       style="width: 48px; font-size: 10.5px; padding: 2px; background: rgba(0,0,0,0.3); color: #fff; border: 1px solid rgba(255,255,255,0.15); text-align: center;"
                       oninput="updateAngleFromInput('${d.key}', this.value)">
            </div>
        </div>
        `;
    });
...
window.updateAngleFromSlider = function(key, val) {
    ...
    const inputEl = document.getElementById(`input-angle-${key}`);
    if (inputEl) inputEl.value = angle;

    // Update text label
    const txtEl = document.getElementById(`txt-angle-${key}`);
    if (txtEl) txtEl.innerText = `${angle}°`;

    // Update map slot rotation immediately!
...
window.updateAngleFromInput = function(key, val) {
    const jid = STATE.activeJid;
    if (!jid || !STATE.junctions[jid]) return;
    const j = STATE.junctions[jid];
    ...
    // Normalize N inputs: e.g. 333..360 -> -27..0
    let sliderVal = angle;
    if (key === 'N') {
        if (sliderVal > 180) sliderVal -= 360;
        // Constraint check for N
        if (sliderVal < -27 || sliderVal > 27) return;
    } else {
        const constraints = {
            'NE': [28, 62], 'E': [63, 117], 'SE': [118, 152],
            'S': [153, 207], 'SW': [208, 242], 'W': [243, 297], 'NW': [298, 332]
        };
        const range = constraints[key];
        if (angle < range[0] || angle > range[1]) return;
    }
    ...
    // Update slider value
    const sliderEl = document.getElementById(`slider-angle-${key}`);
