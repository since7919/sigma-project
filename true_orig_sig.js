��export function parsePhaseCode(code) {
  ...
  let typeName = '����???;
  if (typeChar === 'S') typeName = '����J�(1)';
  else if (typeChar === 'L') typeName = '����v�??2)';
  else if (typeChar === 'P') typeName = 'B����(3)';
  else return {
    direction: '����???,
    outputType: '����???,
    pedestrian: 0,
  ...
  if (!isNaN(enterAngle)) {
    ...
    if (angle >= 337 || angle < 22) dirName = 'z�?;
    else if (angle >= 22 && angle < 67) dirName = 'z���޸';
    else if (angle >= 67 && angle < 112) dirName = '??;
    else if (angle >= 112 && angle < 157) dirName = '?d$޸';
    else if (angle >= 157 && angle < 202) dirName = '??;
    else if (angle >= 202 && angle < 247) dirName = '?e$L�';
    else if (angle >= 247 && angle < 292) dirName = '??;
    else if (angle >= 292 && angle < 337) dirName = 'z���L�';
  }

  const dirAngleMap = { 'z�?: 0, 'z���޸': 45, '??: 90, '?d$޸': 135, '??: 180, '?e$L�': 225, '??: 270, 'z���L�': 315 };
  ...
  return allMovs.map(m => {
    ...
    if (isSeoul) {
      ...
      if (spat && spat.status) {
        ...
        if (isPed) {
          if (pdsg === 'protected-Movement-Allowed' || pdsg === 'permissive-Movement-Allowed' || pdsg === '?@�K�') { 
            signalState = 'G';
            if (pdTime) countdown = Math.floor(pdTime / 10);
          } else if (pdsg === 'protected-clearance' || pdsg === '?y$K�') {
            signalState = 'F';
            if (pdTime) countdown = Math.floor(pdTime / 10);
          }
        } else {
          ...
          if (isLeftMov) {
            if (ltsg === 'protected-Movement-Allowed' || ltsg === '?@�K�?����??) {
              signalState = 'G';
              if (ltTime) countdown = Math.floor(ltTime / 10);
            } else if (ltsg === 'protected-clearance' || ltsg === '?y$K�') {
              signalState = 'Y';
              if (ltTime) countdown = Math.floor(ltTime / 10);
            }
          } else {
            if (stsg === 'protected-Movement-Allowed' || stsg === 'permissive-Movement-Allowed' || stsg === '?@�K�' || stsg === '?@�K�?����?? || stsg === '��?K�') {
              signalState = 'G';
              if (stTime) countdown = Math.floor(stTime / 10);
            } else if (stsg === 'protected-clearance' || stsg === 'permissive-clearance' || stsg === '?y$K�') {
              signalState = 'Y';
              if (stTime) countdown = Math.floor(stTime / 10);
            }
          }
  ...
  return directions.map(({ key, deg }) => {
    ...
    if (isSeoul) {
      ...
      if (spat && spat.status) {
        ...
        if (!stOn && !ltOn && (stsg === 'stop-And-Remain' || ltsg === 'stop-And-Remain' || stsg === '?��K�' || ltsg === '?��K�' || s === 'off')) { 
          s = 'red'; 
          l = 'red'; 
        }
        ...
        if (pdsg === 'protected-Movement-Allowed' || pdsg === 'permissive-Movement-Allowed' || pdsg === '?@�K�') { 
          ...
          p = 'flash';
          if (pdTime) pedCountdown = Math.max(pedCountdown, Math.floor(pdTime / 10));
        } else if (pdsg === 'stop-And-Remain' || pdsg === '?��K�' || p === 'off') { 
          p = 'red'; 
        }
      } else {
